...
{
  dMass m1;
  dReal radius = 0.1, length = 0.5, mass = 2.0; 
  pillar.body = dBodyCreate(world);
  dMassSetZero(&m1);
  dMassSetCylinderTotal(&m1,mass,3,radius,length);
  dBodySetMass(pillar.body,&m1);
  dBodySetPosition(pillar.body,0.0,0.0,1.0); 
  dBodySetLinearVel(pillar.body,0.0, 0.0, 1.0); 

  dMatrix3 R;
  dRFromAxisAndAngle(R,1.0,0.0,0.0, M_PI/4.0);
